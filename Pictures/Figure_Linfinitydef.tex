			...
			\tikz[na]\node [scale=0.5,coordinate,fill=blue!20,draw,circle] (n2) {};	    
				...
				\begin{itemize}
				    	...
				    	\item[-] $\textrm{deg}(\mu_k)= k - 2$
							...
							$$\displaystyle J_m=\sum_{i+j=m+1}(-)^{i(j+1)} 	\mu_i \circ \mu_j = 0
							\quad 							(\forall m\geq 1)$$		
				\end{itemize}		
		\end{minipage}

		\footnotetext{Notation: $\qquad\mu_i\circ \mu_j := \frac{1}{j!(i-1)!}\mu_i\cdot\mu_j \cdot \mathcal{A}~, \qquad \mathcal{A} =$ (graded) total skew-symmetrizator.}
	\end{minipage}
